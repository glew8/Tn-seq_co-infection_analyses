...
# For Tn5 or other "randomly" inserting transposons
##################################################################################################################
#Clear environment
rm(list=ls())
##################################################################################################################
...
gff_dir
#set the length of your genome in base pairs
genomelength = 6537648
genomelength
...
num_expected
#set the file prefix
out_pfx = paste(ctrl_pfx, "Essential_1000", sep = "_")
out_pfx
...
setwd(wd)
print(head(gff))
#gff <- tail(gff, n=-9) #uncomment if gff has extra info at top, change to the number of extra lines to remove
...
for (i in 1:num_expected) {
  expected <- data.frame(Pos=sample(1:genomelength, numsites), Exp=sample(sites2$mean, numsites)) %>% arrange(Pos)
